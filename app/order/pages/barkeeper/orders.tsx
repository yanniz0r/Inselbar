...
const Orders: FC = () => {
  ...
  const prepare = (order: Order) => async () => {
    const result = await updateOrderMutation({ where: { id: order.id }, data: { status: OrderStatus.InProgress }});
    await ordersQuery.refetch();
  }

  const complete = (order: Order) => async () => {
    const result = await updateOrderMutation({ where: { id: order.id }, data: { status: OrderStatus.Done }});
    await ordersQuery.refetch();
  }

  return <table className="table-auto w-full border-t-2 border-gray-200">
    {orders.map((order) => (
      <tr className="border-top-2 border-grey-500 w-full border-gray-200 border-b-2">
        ...
        <td>
          for {order.user.username}
        </td>
        <td className="py-4 flex justify-end">
          {order.status === OrderStatus.InProgress
            ? <button type="button" className="bg-green-500 text-white rounded-lg p-1"  onClick={complete(order)}>Finish</button>
            : <button type="button" className="bg-pink-600 text-white rounded-lg p-1" onClick={prepare(order)}>Prepare</button>
          }
        </td>
      </tr>
    ))}
