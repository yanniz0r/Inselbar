...
const OrderStatusInfos: FC = () => {
  ...
  return <>
    ...
    <h3 className="my-3 text-center">We received your order and will prepare your drink for you. You can already get excited for your...</h3>
    {order.product.image &&
      <div className="flex justify-center">
        <div className="h-32 w-32 bg-red-500 bg-cover bg-center rounded-full" style={{backgroundImage: `url(${order.product.image})`}} />
      </div>
    }
        ...
        <div className="absolute w-full transform -translate-y-5">
          <div className="absolute flex flex-col items-center transform -translate-x-1/2">
            ...
            <p className="text-xs text-center">
              We received<br/> your order
            </p>
          </div>
          <div className="absolute flex flex-col items-center left-1/2 transform -translate-x-1/2">
            <div className={`mb-2 h-8 w-8 text-black text-opacity-40 flex justify-center items-center ${order.status !== OrderStatus.Submitted ? 'bg-green-400' : 'bg-gray-200'} rounded-full`}>
              <GiLemon />
            </div>
            <p className="text-xs text-center">
              Your drinks<br/>are being mixed
            </p>
          </div>
          <div className="absolute flex flex-col items-center left-full transform -translate-x-1/2">
            <div className={`mb-2 h-8 w-8 text-black text-opacity-40 flex justify-center items-center ${order.status === OrderStatus.Done ? 'bg-green-400' : 'bg-gray-200'} rounded-full`}>
              <FaCocktail />
            </div>
            <p className="text-xs text-center">
              Your order is ready
